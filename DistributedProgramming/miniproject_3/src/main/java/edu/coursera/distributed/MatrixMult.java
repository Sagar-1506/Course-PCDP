...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        int totalRows = a.getNRows();

        int rowsPerNode = (totalRows + size - 1) / size;

        int startRow = rank * rowsPerNode;

        int endRow = Math.min((rank + 1) * rowsPerNode, totalRows);
        ...
        if (rank == 0) {
            ...
            int totalColumns = c.getNCols();

            for (int i = 1; i < size; i++) {

                int outputStartRow = i * rowsPerNode;
                int outputEndRow = Math.min((i + 1) * rowsPerNode, totalRows);

                int offset = outputStartRow * totalColumns;

                int numElements = (outputEndRow - outputStartRow) * totalColumns;

                requests[i - 1] = mpi.MPI_Irecv(c.getValues(), offset, numElements, i, i, mpi.MPI_COMM_WORLD);
            }

            mpi.MPI_Waitall(requests);
        } else {
            mpi.MPI_Send(c.getValues(), startRow * c.getNCols(), (endRow - startRow) * c.getNCols(), 0, rank, mpi.MPI_COMM_WORLD);
        }
