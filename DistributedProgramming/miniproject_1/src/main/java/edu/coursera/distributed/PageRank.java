...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> targetContr = sites.join(ranks).flatMapToPair(kv -> {

            Double currRank = kv._2()._2();
            Website website = kv._2()._1();

            Iterator<Integer> edgeIterator = website.edgeIterator();

            int totalEdges = website.getNEdges();

            List<Tuple2<Integer, Double>> contributions = new LinkedList<>();

            while (edgeIterator.hasNext()) {
                final int targetSite = edgeIterator.next();
                contributions.add(new Tuple2<>(targetSite, currRank * 1.0 / totalEdges));
            }

            return contributions;
        });

        return targetContr.reduceByKey((Double contr1, Double contr2) -> contr1 + contr2).mapValues(v -> 0.15 + 0.85 * v);
    }
}
