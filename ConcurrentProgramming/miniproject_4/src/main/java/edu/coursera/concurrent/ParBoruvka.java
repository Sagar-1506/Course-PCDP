    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded,
        ...
        for (int i = 0; i < cores; i++) {
            threads[i] = new Thread(() -> {
                ParComponent currNode;

                while ((currNode = nodesLoaded.poll()) != null) {

                    if (currNode.isDead) {
                        continue;
                    }

                    if (!currNode.lock.tryLock()) {
                        continue;
                    }
                    ...
                    Edge<ParComponent> edge = currNode.getMinEdge();
                    ...
                    ParComponent otherNode = edge.getOther(currNode);


                    if (otherNode == null || otherNode.isDead) {
                        currNode.lock.unlock();
                        nodesLoaded.add(currNode);
                        continue;
                    }
                    ...
                    currNode.merge(otherNode, edge.weight());
