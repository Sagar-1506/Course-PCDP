...
public final class MatrixMultiply {
    ...
    public static void parMatrixMultiply(final double[][] A, final double[][] B,
        ...
        IntStream.range(0, N * N)
                .parallel()
                .forEach(index -> calculateMatrixProduct(A, B, C, N, index));

    }

    private static void calculateMatrixProduct(final double[][] A, final double[][] B,
                                               final double[][] C, final int N, int index) {
        int row = index / N;
        int col = index % N;
