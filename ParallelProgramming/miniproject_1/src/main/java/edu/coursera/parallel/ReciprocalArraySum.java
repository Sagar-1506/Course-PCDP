...
public final class ReciprocalArraySum {
    ...
    protected static double parManyTaskArraySum(final double[] input,
        ...
        for (int i = 0; i < numTasks - 1; i++) {
            ...
            int start = getChunkStartInclusive(i, numTasks, totalElements);
            int end = getChunkEndExclusive(i, numTasks, totalElements);

            tasks[i] = new ReciprocalArraySumTask(start, end, input);
            pool.execute(tasks[i]);
        }

        int lastStart = getChunkStartInclusive(numTasks - 1, numTasks, totalElements);
        int lastEnd = getChunkEndExclusive(numTasks - 1, numTasks, totalElements);

        tasks[numTasks - 1] = new ReciprocalArraySumTask(lastStart, lastEnd, input);
